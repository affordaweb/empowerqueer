    ...
    <footer className="bg-white border-t border-gray-200 pt-16 pb-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 mb-12">
          ...
          {/* Quick Links */}
          <div>
            <h4 className="font-semibold text-[#3D3558] text-sm mb-4 uppercase tracking-wider">
              Quick Links
            </h4>
            <ul className="space-y-2">
              {quickLinks.map((link) => (
                <li key={link.href}>
                  <a
                    href={link.href}
                    className="text-[#52506A] hover:text-[#3D3558] text-sm transition-colors flex items-center gap-1.5 group"
                  >
                    <ChevronRight
                      size={12}
                      className="opacity-0 group-hover:opacity-100 transition-opacity -ml-2 shrink-0 text-[#3D3558]"
                    />
                    {link.label}
                  </a>
                </li>
            ...
            <ul className="space-y-4">
              {recentPosts.map((post) => (
                <li key={post.title}>
                  <a
                    href={post.href}
                    className="text-[#52506A] hover:text-[#3D3558] text-sm transition-colors leading-relaxed block"
                  >
                    {post.title}
                  </a>
                </li>
